...
void main() {
	...
	// Output the components of the rotation matrix to ensure they are correct
	//FragColor = vec4(rotmat[0], 1.0);  // Visualize T (red channel)
...
out vec3 v_position;
out vec3 v_normal;
out vec2 v_uv; // Add v_uv as a varying output

void main()
{
    // Transform the vertex to world space
    vec4 world_position = u_model * vec4(a_vertex, 1.0);
    
    // Transform the normal to world space (no translation)
    v_normal = (u_model * vec4(a_normal, 0.0)).xyz;

    // Pass the transformed vertex and normal to the fragment shader
    v_position = world_position.xyz;

    // Assign UV coordinates to v_uv
    v_uv = vec2((a_vertex.x + 1.0) * 0.5, (a_vertex.y + 1.0) * 0.5); // Assuming a simple UV mapping for the sphere
...
void main()
{
	...
	FragColor = vec4(color.xyz, metalness);
	
	//Si estoy en la escena de irradiance utilizo la normal N.
    ...
    if (u_emissive_texture_enabled)
		emissive *= texture(u_emissive_texture, uv).xyz;
